...
class HttpClientSpec extends AnyFlatSpec with Matchers {
  ...
  it should "build a left if 400" in {
    ...
    val response = Response[IO](status = Status.BadRequest, body = createBody(bre.asJson))
    val res      = HttpClient.buildResponse[IO, Foo](response)
  ...
  it should "build a left if 401" in {
    ...
    val response = Response[IO](status = Status.Unauthorized, body = createBody(ue.asJson))
    val res      = HttpClient.buildResponse[IO, Foo](response)
  ...
  it should "build a left if 403" in {
    ...
    val response = Response[IO](status = Status.Forbidden, body = createBody(fe.asJson))
    val res      = HttpClient.buildResponse[IO, Foo](response)
  ...
  it should "build a left if 404" in {
    ...
    val response1 = Response[IO](status = Status.NotFound, body = createBody(nfe.asJson))
    ...
    val response2 = Response[IO](status = Status.NotFound, body = createBody(be.asJson))
  ...
  it should "build a left if 422" in {
    ...
    val response = Response[IO](status = Status.UnprocessableEntity, body = createBody(uee.asJson))
    val res      = HttpClient.buildResponse[IO, Foo](response)
  ...
  it should "build a left if 423" in {
    ...
    val response = Response[IO](status = Status.Locked, body = createBody(rlee.asJson))
    val res      = HttpClient.buildResponse[IO, Foo](response)
  ...
  it should "build a left if unhandled status code" in {
    val rlee     = RateLimitExceededError("msg", "")
    val response = Response[IO](status = Status.PaymentRequired, body = createBody(rlee.asJson))
    val res      = HttpClient.buildResponse[IO, Foo](response)
